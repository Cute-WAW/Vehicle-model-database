...
def create_patent_doc_v2():
    doc = Document()
    
    # --- Title ---
    # 标题：一种基于多源特征检索与自适应非线性回归的二手车残值预测方法
    title_para = doc.add_paragraph()
    title_para.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = title_para.add_run('一种基于多源特征检索与自适应非线性回归的二手车残值预测方法')
    ...
    abstract_text = (
        "本发明公开了一种基于多源特征检索与自适应非线性回归的二手车残值预测方法。该方法首先通过实体提取技术构建包含年款、排量、变速箱及版式的多维车辆特征索引；"
        "针对新车价格缺失问题，提出五级特征匹配推断策略；"
        "在预测阶段，采用竞争优选机制为不同品牌车系及车龄段自动匹配最佳非线性回归模型（指数、多项式或幂函数）；"
        "进而利用基于特征相似度的动态置信度算法，融合相近车辆的市场成交价对模型预测结果进行修正；"
        "最后结合C2B2C模型生成多场景价格矩阵。本发明有效解决了传统模型对非标品特征敏感度低、新车基准缺失及长尾车型预测难的问题，显著提升了二手车估值的准确性与鲁棒性。"
    )
    ...
    p = doc.add_paragraph(
        "本发明涉及汽车数据挖掘与价值评估技术领域，具体涉及一种基于多源特征检索与自适应非线性回归的二手车残值预测方法。"
    )
    p.paragraph_format.first_line_indent = Inches(0.3)
    set_font(p.runs[0], '宋体', Pt(12))
    ...
    background_text = (
        ...
        "2. 模型泛化能力差：传统的线性回归难以拟合二手车价格随车龄非线性衰减的规律，而复杂的机器学习模型（如XGBoost）在小样本（长尾车型）上容易过拟合。\n"
    ...
    p = doc.add_paragraph(
        "本发明的目的在于提供一种基于多源特征检索与自适应非线性回归的二手车残值预测方法，通过多层级的特征匹配与动态模型优选，解决上述技术问题。"
    )
    p.paragraph_format.first_line_indent = Inches(0.3)
    set_font(p.runs[0], '宋体', Pt(12))
    ...
    claims = [
        "1. 一种基于多源特征检索与自适应非线性回归的二手车残值预测方法，其特征在于，包括以下步骤：\n"
        ...
        "S3：自适应非线性模型预测。根据待预测车辆的品牌车系及车龄段，自动优选拟合度最高的非线性回归模型（指数/多项式/幂函数）进行基础预测；若样本不足，则自动回退至车辆类别通用模型；\n"
        ...
        "3. 根据权利要求1所述的方法，其特征在于，步骤S3中的自适应非线性模型包括：\n"
        "指数模型：y = a * e^(b*x)\n"
        "二次多项式模型：y = a*x^2 + b*x + c\n"
        "幂函数模型：y = a * x^b\n"
        "其中y为残值率，x为车龄。系统对每个品牌车系分别训练上述三种模型，并选择R²分数最高者作为最终模型。",
        
        "4. 根据权利要求1所述的方法，其特征在于，步骤S4中的修正公式为：\n"
        "P_final = P_model * (1 + Deviation * Confidence * Factor)\n"
        "其中P_model为模型预测价，Deviation为(相近车辆均价 - P_model)/P_model，Confidence为相近车辆平均相似度得分归一化值，Factor为调节因子。"
    ]
    ...
    # 四、具体实施方式
    head_para = doc.add_paragraph()
    run = head_para.add_run('四、具体实施方式')
    set_font(run, '黑体', Pt(14), True)
    ...
    sections = [
        ...
        ("2. 自适应非线性回归建模", 
         "针对二手车残值率随车龄非线性衰减的特性，本系统摒弃了单一模型策略，采用“竞争优选”机制：\n"
         "对于每个品牌车系（如“本田-飞度”），系统分别尝试拟合指数模型、二次多项式模型和幂函数模型。通过计算R²（决定系数）和RMSE（均方根误差），自动选择表现最优的模型并序列化保存。\n"
        ...
        ("3. 动态置信度修正算法", 
         ...
         "系统实时检索与待预测车辆特征最相似的Top-30个历史成交案例。对于每个案例，根据其与目标车辆在年款、排量、变速箱、版式、里程、城市等维度的匹配程度，计算相似度得分（Score）。\n"
    ...
    # Save
    output_path = os.path.join(os.getcwd(), 'output', 'patent_v2.docx')
    if not os.path.exists(os.path.dirname(output_path)):
        os.makedirs(os.path.dirname(output_path))
        
    doc.save(output_path)
    print(f"Patent document v2 generated at: {output_path}")

if __name__ == "__main__":
    create_patent_doc_v2()
